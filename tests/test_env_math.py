...
def test_imitation_reward_uniform_golden():
    ...
    np.testing.assert_allclose(reward, REWARD_GOLDEN["reward_uniform"], rtol=1e-10)
    np.testing.assert_allclose(raw["r_body_pos"], REWARD_GOLDEN["r_body_pos_uniform"], rtol=1e-10)
    np.testing.assert_allclose(raw["r_vel"], REWARD_GOLDEN["r_vel_uniform"], rtol=1e-10)
...
def test_imitation_reward_weighted_golden():
    ...
    np.testing.assert_allclose(reward, REWARD_GOLDEN["reward_weighted"], rtol=1e-10)
    np.testing.assert_allclose(raw["r_body_pos"], REWARD_GOLDEN["r_body_pos_weighted"], rtol=1e-10)
...
def test_imitation_reward_perfect_match_is_max():
    ...
    reward, raw = compute_imitation_reward(
        body_pos, body_vel, body_pos.copy(), body_vel.copy(), H.REWARD_SPECS,
    )
    np.testing.assert_allclose(raw["r_body_pos"], 1.0, atol=1e-12)
    np.testing.assert_allclose(raw["r_vel"], 1.0, atol=1e-12)
    np.testing.assert_allclose(reward, H.REWARD_SPECS["w_pos"] + H.REWARD_SPECS["w_vel"], atol=1e-12)
...
def test_imitation_reward_matches_closed_form():
    ...
    np.testing.assert_allclose(
        reward, H.REWARD_SPECS["w_pos"] * exp_pos + H.REWARD_SPECS["w_vel"] * exp_vel, rtol=1e-10,
    )


def test_imitation_reward_uniform_weights_equals_none():
    body_pos, body_vel, ref_pos, ref_vel = H.imitation_reward_inputs()
    r_none, _ = compute_imitation_reward(body_pos, body_vel, ref_pos, ref_vel, H.REWARD_SPECS)
    w = np.full(H.NUM_TRACKED_BODIES, 1.0 / H.NUM_TRACKED_BODIES)
    r_w, _ = compute_imitation_reward(body_pos, body_vel, ref_pos, ref_vel, H.REWARD_SPECS, body_weights=w)
...
def test_imitation_observations_golden():
    ...
    for key in ("diff_local_body_pos", "diff_local_vel", "local_ref_body_pos",
                "diff_muscle_len", "diff_muscle_vel"):
        np.testing.assert_allclose(obs[key], OBS_GOLDEN[key], rtol=1e-10, atol=1e-12)


def test_imitation_observations_zero_when_matched():
    (root_pos, root_rot, body_pos, body_vel, ref_pos, ref_vel,
     mlen, mvel, rmlen, rmvel) = H.imitation_obs_inputs()
    obs = compute_imitation_observations(
        root_pos, root_rot, body_pos, body_vel, body_pos.copy(), body_vel.copy(),
        mlen, mvel, mlen.copy(), mvel.copy(), time_steps=1,
    )
...
def test_imitation_observations_muscle_diff_sign():
    ...
    obs = compute_imitation_observations(*inp, time_steps=1)
    # diff = ref - current
    np.testing.assert_allclose(obs["diff_muscle_len"], inp[8] - inp[6], rtol=1e-12)
    np.testing.assert_allclose(obs["diff_muscle_vel"], inp[9] - inp[7], rtol=1e-12)


# ─────────────────────────── reset / termination ───────────────────
def test_im_reset_golden():
    body, close, far = RESET_GOLDEN["body"], RESET_GOLDEN["ref_close"], RESET_GOLDEN["ref_far"]
    thr = RESET_GOLDEN["per_body_thresh"]
    np.testing.assert_array_equal(compute_humanoid_im_reset(body, close, 0.15, False), RESET_GOLDEN["close_max"])
    np.testing.assert_array_equal(compute_humanoid_im_reset(body, close, 0.15, True), RESET_GOLDEN["close_mean"])
    np.testing.assert_array_equal(compute_humanoid_im_reset(body, far, 0.15, False), RESET_GOLDEN["far_max"])
    np.testing.assert_array_equal(compute_humanoid_im_reset(body, far, 0.15, True), RESET_GOLDEN["far_mean"])
    np.testing.assert_array_equal(compute_humanoid_im_reset(body, far, thr, False), RESET_GOLDEN["far_max_perbody"])
...
def test_im_reset_far_body_triggers_max_not_necessarily_mean():
    ...
    assert compute_humanoid_im_reset(body, far, 0.15, False)[0]  # one body over threshold
...
def test_self_observations_golden():
    ...
    for key in ("root_h_obs", "local_body_pos", "local_body_rot_obs",
                "local_body_vel", "local_body_ang_vel"):
        np.testing.assert_allclose(obs[key], SELF_GOLDEN[key], rtol=1e-10, atol=1e-12)
...
def test_self_observations_root_height_is_root_z():
    body_pos, body_rot, body_vel, body_ang_vel = H.self_obs_inputs()
    obs = compute_self_observations(body_pos, body_rot, body_vel, body_ang_vel)
    np.testing.assert_allclose(obs["root_h_obs"][0, 0], body_pos[0, 0, 2], rtol=1e-12)
